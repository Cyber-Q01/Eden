...
12. ☐ Env on `send-ad-notification`: **none required** — it uses the same Expo endpoint as your `send-push-notification` (no auth). Optional: `EXPO_PUSH_ACCESS_TOKEN` only if you later enable "enhanced push security" in your Expo dashboard. That's the whole setup — no secrets, no copy-paste values. New ads push automatically within 15 min of going active (or press Invoke to push now). Check reach: `select title, push_sent_at, push_users from public.advertisements order by created_at desc limit 5;`
...
15. ☐ **Escrow release (rent money → landlord):** redeploy `release-payment` (v2 in this zip — it now verifies the transfer itself via Paystack's `GET /transfer/verify` and releases the rental inside the function; env `PAYSTACK_SECRET_KEY`) and `paystack-webhook` (v2 — no longer drops late `transfer.success` events; it's the backup for queued live transfers). Then run `20260828010000_payout_active_unique.sql` (hard double-payment guard: only one pending/success payout per rental). OPTIONAL: `supabase functions deploy auto-release-escrow --no-verify-jwt --cron "*/15 * * * *"` — only if you want a scheduled fallback release; skip it if release-payment + webhook is your flow.
