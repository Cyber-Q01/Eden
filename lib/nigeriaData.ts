...
export const NIGERIA_STATES_LGAS: StateData[] = [
    {
        state: 'Abia',
    ...
    {
        state: 'Edo',
        lgas: ['Akoko-Edo', 'Benin', 'Egor', 'Esan Central', 'Esan North-East', 'Esan South-East', 'Esan West', 'Etsako Central', 'Etsako East', 'Etsako West', 'Igueben', 'Ikpoba Okha', 'Orhionmwon', 'Oredo', 'Ovia North-East', 'Ovia South-West', 'Owan East', 'Owan West', 'Uhunmwonde'],
    },
    {
        state: 'Ekiti',
...
export const getStateNames = (): string[] => NIGERIA_STATES_LGAS.map(s => s.state);

export const getLGAsForState = (stateName: string): string[] => {
    const found = NIGERIA_STATES_LGAS.find(s => s.state === stateName);
